...
var nearby_snapping_point: Node3D = null
...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("interact"):
		if held_item:
			...
			nearby_item = find_nearby_target("Pickupable", 2.0)
			if nearby_item:
...
func pickup_item(item):
	if held_item and nearby_snapping_point.item_present == false:
		drop_item()
	...
	var saved_global = held_item.global_transform # To Save Match Held Item Scale For Later Use.
...
func drop_item():
	...
	nearby_snapping_point = find_nearby_target("SnappingPoints", 2.0)
	if nearby_snapping_point and nearby_snapping_point.preparing == false:
		try_placing_item()
		
# 	else: # Overcooked Logic, Probably Don't Need it Design Wise.
	# if held_item is RigidBody3D:
		# held_item.freeze = false
		# held_item.freeze_mode = RigidBody3D.FREEZE_MODE_STATIC
		# held_item.global_transform = saved_global

func try_placing_item():
	if held_item and nearby_snapping_point.item_present == false:
...
func snap_to_position(_snap_pos):
