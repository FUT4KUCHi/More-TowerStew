...
func _ready():
	...
	customer = assign_random_customer_data(level.customer_registry, level.item_registry)
...
func check_order_item() -> void:
	if has_ordered and current_table.item_stored:
		if current_table.item_stored and current_table.stored_item_id == order_item.id:
			print("Satisfied.")
			queue_free()
			current_table.remove_item_from_self()
			seating_system.release_chair(current_chair)
			# TODO -> Update this later.
			get_parent().find_child("Points").text = "Points: " + str(get_parent().find_child("Player").point_increase())
		elif !current_table.item_stored and current_table.stored_item_id != order_item.id:
